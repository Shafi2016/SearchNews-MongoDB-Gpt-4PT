...
# # if not OPENAI_API_KEY or not MONGODB_URL:
# #     st.error("Please ensure OPENAI_API_KEY and MONGODB_URL are set in your .env file.")
# #     st.stop()
...
# #             st.write(f"Number of articles after deduplication: {len(df)}")
...
# #                     st.write(f"\nNumber of articles selected: {len(selected_indices)}")
...
# #                 # Summary: Cost of Living and Economic Concerns in Canada
...
# #                 Use only the information provided in the context. Do not include any information that is not from these articles. Make sure to reference ALL provided articles in your summary.
# #                 """
...
# #             # Generate the summary report using the multi-agent system
# #             final_answer = agent_executor.run(query)

# #             # Display the final summary report
# #             # st.markdown(final_answer)
# #             # Use regex to extract the "Observation" and "Final Answer" sections
# #             observation_match = re.search(r'## Observation\s*(.*?)\s*## Final Answer', final_answer, re.DOTALL)
# #             final_answer_match = re.search(r'## Final Answer\s*(.*)', final_answer, re.DOTALL)

# #             if observation_match and final_answer_match:
# #                 observation = observation_match.group(1).strip()
# #                 final_ans = final_answer_match.group(1).strip()

# #                 # Display the Observation section
...
# #             else:
# #                 # Fallback: Display the entire answer if parsing fails
# #                 st.markdown(final_answer)
